...
fn command_emulate(config: &Config, contract_id: Option<H160>, caller_id: H160, data: Option<Vec<u8>>) {
    ...
    let (exit_reason, result, applies_logs, used_gas) = {
        ...
        let (exit_reason, result) = match &contract_id {
            Some(contract_h160) =>  {
                executor.transact_call(caller_id, *contract_h160, U256::zero(), data.unwrap_or_default(), u64::MAX)
            },
            None => {
                (executor.transact_create(caller_id, U256::zero(), data.unwrap_or_default(), u64::MAX), Vec::new())
            }
        };
