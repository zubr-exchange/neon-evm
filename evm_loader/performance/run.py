...
def mint_erc20_create(accounts, acc, sum):
...
def mint_erc20(accounts, acc):
    sum = 1000 * 10 ** 18
    receipt_list = mint_erc20_create(accounts, acc, sum)
    return mint_erc20_confirm(receipt_list, sum)
...
def create_accounts(args, type=transfer_type.erc20):
    ...
    if type==transfer_type.spl:
        ...
        # erc20.mint()
        (account_minted, total, event_error, receipt_error, nonce_error, unknown_error, too_small_error) = mint_erc20(ether_accounts, instance.acc)
